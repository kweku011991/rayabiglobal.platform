...
function DashboardPage() {
  const { newRequest } = Route.useSearch();
  // Initialize sessionId once and keep it stable
    ...
    <div className="min-h-screen bg-[#f3f7f3] pb-20 font-sans text-[#1b2b1b]">
      ...
      <React.Suspense fallback={
        <main className="container mx-auto px-6 py-12 flex items-center justify-center min-h-[60vh]">
          <div className="text-[#9caf9c] font-black uppercase tracking-widest animate-pulse">Syncing with Global Hub...</div>
        </main>
      }>
        <DashboardContent 
          sessionId={sessionId} 
      ...
      {/* Request Modal - Outside Suspense to keep state during data syncs */}
      {isRequestModalOpen && (
...
function DashboardContent({ 
  ...
  const { data: myRequests } = useSuspenseQuery(
    convexQuery(api.requests.listByUser, { sessionId })
  );
  const { data: myOrders } = useSuspenseQuery(
    convexQuery(api.orders.listByUser, { sessionId })
  );
    ...
    <main className="container mx-auto px-6 py-12 space-y-16 relative">
      ...
      <section>
        <div className="flex items-center gap-4 mb-8">
          <h2 className="text-2xl font-black uppercase tracking-tight">Direct Shop</h2>
          <div className="h-px flex-1 bg-[#9caf9c]/20"></div>
        </div>
        {products.length === 0 ? (
          <p className="text-[#9caf9c] bg-white p-12 rounded-2xl border border-[#9caf9c]/10 text-center font-bold uppercase text-xs tracking-widest">Scanning Global Inventory...</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {products.map((product) => (
              <div key={product._id} className="bg-white rounded-2xl overflow-hidden hover:shadow-2xl transition-all border border-[#9caf9c]/10 group flex flex-col h-full">
                ...
                <div className="p-8 flex flex-col flex-1">
                  <div className="mb-4">
                    ...
                    <div className="flex gap-3 text-[10px] font-bold text-[#9caf9c] uppercase tracking-widest">
                  ...
                  <div className="flex justify-between items-end mt-auto pt-6 border-t border-[#f3f7f3]">
                    ...
                    <button 
                      ...
                      className="bg-[#1b2b1b] text-white px-6 py-2 rounded-sm font-black text-[10px] uppercase tracking-widest hover:bg-[#9caf9c] transition-all"
                    >
                      Buy Now
                    </button>
                  </div>
